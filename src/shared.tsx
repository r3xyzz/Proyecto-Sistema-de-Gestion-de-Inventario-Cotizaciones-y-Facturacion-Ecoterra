...
export function Ico({ p, size = 16, stroke = 1.75 }: { p: string; size?: number; stroke?: number }) {
  return (
...
export type Screen = "dashboard" | "clientes" | "proveedores" | "productos" | "lotes" | "movimientos" | "cotizaciones" | "oc" | "facturacion" | "nc" | "ia";
export type Product = { id: number; codigo: string; nombre: string; desc: string; tipo: string; unidad: string; stockMin: number; stock: number };
...
export function fmt(n: number) { return n.toLocaleString("es-CL"); }
...
export function PageTitle({ title, sub, children }: { title: string; sub?: string; children?: ReactNode }) {
  return <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}><div><h1 style={{ fontSize: "1.25rem", fontWeight: 700, color: "#0F172A", letterSpacing: "-0.01em", lineHeight: 1.2 }}>{title}</h1>{sub && <p style={{ fontSize: "0.75rem", color: "#64748B", marginTop: 2 }}>{sub}</p>}</div>{children && <div style={{ display: "flex", gap: 8 }}>{children}</div>}</div>;
}
